...
def get_db_credentials(hostip=tvaultconf.tvault_ip):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.load_system_host_keys()
    ssh.connect(hostname=hostip, username=tvaultconf.tvault_dbusername,
                password=tvaultconf.tvault_dbpassword)
